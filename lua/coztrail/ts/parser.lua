...
function M.get_function_at_pos(bufnr, pos)
  ...
  logger.debug(
    string.format('Searching for function node from position [%d, %d]', row, col),
    'parser'
  )
  ...
  while node do
    if function_node_types[node:type()] then
      local start_row, _, end_row, _ = node:range()
      logger.info(
        string.format('Function/method found at lines %d-%d', start_row, end_row),
        'parser'
      )
      local lines = vim.api.nvim_buf_get_lines(bufnr, start_row, end_row + 1, false)
      return table.concat(lines, '\n'), node
  ...
  logger.warn(
    string.format('No function found at (file: %s, line: %d, col: %d)', filename, row, col),
    'parser'
  )
  return nil, nil
end
...
function M.analyze_structure(bufnr, func_node)
  ...
  logger.info(
    'Structure analysis completed: found '
      .. #structure.calls
      .. ' function calls and '
      .. #structure.globals
      .. ' global variables',
