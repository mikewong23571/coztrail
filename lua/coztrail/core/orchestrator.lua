...
function M.run()
  ...
  M.expand_call_graph(bufnr, func_node, structure, function(callee_summaries)
    M.summarize_with_cache(bufnr, "current_function", func_text, func_node, structure, callee_summaries, function(summary)
...
function M.summarize_with_cache(bufnr, func_name, func_text, func_node, structure, callee_summaries, callback)
  ...
  runner.summarize(func_name, func_text, structure, callee_summaries, function(result)
    if result.success then
      ...
      if save_success then
        logger.debug("Successfully saved summary to cache for function: " .. func_name, "orchestrator")
      else
...
function M.resolve_function_definition(bufnr, call, on_resolve)
  logger.debug("Resolving definition for function: " .. (call.name or "unknown") .. " at line " .. call.line, "orchestrator")
  ...
  vim.lsp.buf_request(0, "textDocument/definition", {
    ...
    if err or not result or vim.tbl_isempty(result) or not result[1] then
      logger.warn("LSP definition request failed for: " .. (call.name or "unknown") .. ", error: " .. (err and err.message or "no result"), "orchestrator")
      vim.notify("Unable to resolve definition for: " .. (call.name or "unknown"), vim.log.levels.WARN)
      on_resolve({ 
        success = false,
    ...
    local def = result[1]
    if not def or (not def.uri and not def.targetUri) or (not def.range and not def.targetRange) then
      on_resolve({ 
        success = false,
    ...
    if call.language == "go" and uri:match("/go/src/") then
      logger.debug("Skipping Go standard library function: " .. call.name .. " at " .. uri, "orchestrator")
      on_resolve({
        success = true,
        skip = true, 
        type = "stdlib", 
    ...
    if func_node then
      ...
      M.expand_call_graph(nbufnr, func_node, structure, function(callee_summaries)
        M.summarize_with_cache(nbufnr, call.name, func_text, func_node, structure, callee_summaries, function(summary)
          -- summary已经是字典格式，直接传递
          on_resolve(summary)
        end)
      end)
  ...
  for _, call in ipairs(structure.calls) do
    ...
    M.resolve_function_definition(bufnr, call, function(summary)
      ...
      if not summary.success then
        logger.warn("Failed to resolve function: " .. call.name .. ", error: " .. (summary.error or "unknown error"), "orchestrator")
      elseif summary.skip then
        logger.debug("Skipped function: " .. call.name .. ", reason: " .. (summary.message or "unknown"), "orchestrator")
      else
      ...
      logger.debug("Call graph progress: " .. resolved_count .. "/" .. total .. " completed", "orchestrator")
      
      if remaining == 0 then
        logger.info("Call graph expansion completed, resolved " .. resolved_count .. " functions", "orchestrator")
